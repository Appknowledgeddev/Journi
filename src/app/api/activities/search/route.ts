...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        error:
          "Activities API is not configured yet. Add GOOGLE_MAPS_API_KEY to the environment.",
      },
      { status: 500 },
    );
  }
  ...
  try {
    const response = await fetch("https://places.googleapis.com/v1/places:searchText", {
      ...
      body: JSON.stringify({
        textQuery: `best activities and things to do in ${destination}`,
        pageSize: 16,
      }),
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: data.error?.message || "Unable to load activities from Google Places.",
        },
        { status: 502 },
      );
    }

    const activities = (data.places ?? [])
      .filter((place) => activityTypes.has(place.primaryType ?? "") || place.displayName?.text)
      .map((place) => ({
        id: place.id ?? place.displayName?.text ?? crypto.randomUUID(),
        title: place.displayName?.text ?? "Activity idea",
